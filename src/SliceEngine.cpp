...
void SliceEngine::strip_user_content()
{
    ...
    // (5) External file references. These keys never appear in any official
    // Snapmaker preset JSON — they exist only as CLI flags (--load-settings,
    // --load-custom-gcodes, …) consumed by Snapmaker_Orca.cpp at the CLI
    // layer. They are cleared defensively in case a user-supplied 3MF was
    // generated by the CLI and carries these flags into the embedded config.
    // load_slicedata / load_settings are ConfigOptionStrings (multiple file
    // paths allowed); the other two are scalar.
