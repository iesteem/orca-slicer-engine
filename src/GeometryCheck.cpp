...
Issue make_issue(int plate_id, const std::string& object_name, const std::string& code, const std::string& message,
                 const std::string& suggestion = "")
{
    return make_warning(plate_id, code, message, object_name, suggestion);
}

void check_non_manifold(const Slic3r::ModelVolume& volume, const std::string& obj_name, int plate_id,
                        std::vector<Issue>& out)
{
    ...
    if (open_edges > 0)
    {
        ...
        out.push_back(make_issue(plate_id, obj_name, "GEOM_NON_MANIFOLD",
                                 "Non-manifold mesh: " + std::to_string(open_edges) + " open edge(s) out of " +
                                     std::to_string(total_edges) + " total edges (" + pct_buf +
                                     "%). "
                            ...
                            std::vector<Issue>& out)
{
    ...
    if (removed > 0)
    {
        ...
        out.push_back(make_issue(plate_id, obj_name, "GEOM_DEGENERATE",
                                 "Degenerate faces: " + std::to_string(removed) + " zero-area triangle(s) removed (" +
                                     pct_buf + "% of " + std::to_string(total_faces) +
                                     " total faces). "
...
static bool cgal_self_intersect_safe(const Slic3r::TriangleMesh& mesh, bool* crashed)
{
    *crashed = false;
    __try
    {
...
bool check_self_intersect(const Slic3r::ModelVolume& volume, const std::string& obj_name, int plate_id,
                          std::vector<Issue>& out)
{
#ifdef _MSC_VER
    bool crashed = false;
    ...
    if (crashed)
    {
        BOOST_LOG_TRIVIAL(warning) << "GeometryCheck: CGAL self-intersect check crashed (SEH) for \"" << obj_name
                                   << "\" — skipping";
    }
    ...
    {
        BOOST_LOG_TRIVIAL(warning) << "GeometryCheck: CGAL self-intersect check failed for \"" << obj_name
                                   << "\": " << e.what();
    }
    catch (...)
    {
...
void check_multi_component(const Slic3r::ModelVolume& volume, const std::string& obj_name, int plate_id,
                           std::vector<Issue>& out)
{
...
void check_inverted_normals(const Slic3r::ModelVolume& volume, const std::string& obj_name, int plate_id,
                            std::vector<Issue>& out)
{
    ...
    if (vol < 0.0)
    {
        out.push_back(make_issue(plate_id, obj_name, "GEOM_INVERTED",
                                 "Inverted normals: mesh volume is negative (" + std::to_string(vol) +
...
void check_empty(const Slic3r::ModelVolume& volume, const std::string& obj_name, int plate_id, std::vector<Issue>& out)
{
                       ...
                       std::vector<Issue>& out)
{
    ...
    if (!volume.mesh().its.indices.empty() && std::abs(vol) < 1e-10)
    {
        out.push_back(make_issue(plate_id, obj_name, "GEOM_ZERO_VOLUME",
                                 "Zero-volume mesh: the computed volume (" + std::to_string(vol) +
